...
# matches = re.findall(r"mul\(\s*\d{1,3}\s*,\s*\d{1,3}\s*\)", str_input)
# digits = [re.findall(r"\d+", el) for el in matches]
# sum = 0
# for pair in digits:
...
# # print(matches)
# # print(digits)
# # print(sum) # answer for first part
#
#
# #str_input = "xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))"
# matches = re.findall(r"mul\(\s*\d{1,3}\s*,\s*\d{1,3}\s*\)|do\(\)|don't\(\)", str_input)
#
# print(matches)
#
# run = True
...
#         continue
#     elif run:
#         digits = re.findall(r"\d+", el)
#         x = int(digits[0]) * int(digits[1])
#         sum += x
#
# print(sum)  # answer for second part
